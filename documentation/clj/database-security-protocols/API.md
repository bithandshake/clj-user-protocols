  ...
  ([request initial-data {:keys [additional-action-f
   ...
   (let [ip-address (http/request->ip-address request)
        ...
        (cond (not (audit/ip-address-valid? ip-address))                                  {:body :invalid-request/invalid-ip-address             :status 400}
              ...
              :getting-data                            (letfn [(apply-cascade-f [f data] (if f (f  data) data))]
...
```
(ns my-namespace (:require [database-security-protocols.api :refer [get-data]]))

(database-security-protocols.api/get-data ...)
(get-data                                 ...)
```
...
```
(ns my-namespace (:require [database-security-protocols.api :refer [remove-data]]))

(database-security-protocols.api/remove-data ...)
(remove-data                                 ...)
```
  ...
  ([request initial-data {:keys [additional-action-f
   ...
   (let [ip-address (http/request->ip-address request)
        ...
        (cond (not (audit/ip-address-valid? ip-address))                                  {:body :invalid-request/invalid-ip-address             :status 400}
              ...
              (and additional-security-f        (not (additional-security-f)))            {:body :unknown-error/additional-security-stage-failed :status 520}
              (and additional-action-f          (not (additional-action-f)))              {:body :unknown-error/additional-action-stage-failed   :status 520}
              :storing-data                            (letfn [(apply-cascade-f [f data] (if f (f  data) data))]
...
```
(ns my-namespace (:require [database-security-protocols.api :refer [store-data]]))

(database-security-protocols.api/store-data ...)
(store-data                                 ...)
```
